    ...
    <template v-slot:header="{}">
      <div class="user-log">
        <div class="inline-block">
          <v-input theme="box" placeholder="邮箱" v-model="email"></v-input>
        </div>
        <div class="inline-block">
    ...
    <template v-slot:default="{ data }">
      <table class="log-table data-list">
        ...
        <tr v-for="(log, index) in data.records" :key="index">
          ...
          <td>{{ log.operate }}</td>
          <td>{{ log.ip }} {{ log.port ? ':' + log.port : '' }}</td>
          <td>{{ log.result }}</td>
...
export default {
  ...
  methods: {
     ...
     */
    search() {
      if (this.email && !this.$verify('email', this.email)) {
        this.$setMessage({msg: '未识别的邮箱格式', status: false})
        return
      }

      let urlStack = []
      if (this.email) {
        urlStack.push('email=' + this.email)
      }
      if (this.startDate) {
